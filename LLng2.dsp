      ...
      with {
          crossf(i) =
            select2(attPhase(prev)
              ...
              , crossfade(prevH,newH ,ramp(size,reset))*hslider("power %i", 1, 0, 2, 0.01) // TODO crossfade from current direction to new position
            ):min(GR@totalLatency)//TODO: make into brute force fade of 64 samples
          // sample and hold oldDownSpeed:
          , (select2((newDownSpeed > currentDownSpeed),currentDownSpeed ,newDownSpeed))
              with {
              new = lowestGRblock(GR,size)@(i*blockSize);
              newH = new:ba.sAndH(reset);
              prevH = prev:ba.sAndH(reset);
              reset =
                (newDownSpeed > currentDownSpeed) | (attPhase(prev)==0)
              with {
